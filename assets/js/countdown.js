...
IOWA.CountdownTimer.Band.prototype.changeShape = function(t) {
    clearTimeout(this.fadeTimer),
        this.fade("in"),
        this.oldShape = this.currentShape, this.currentShape = t, this.posShift = 0,
...
IOWA.CountdownTimer.Band.prototype.getColor = function(t) {
    ...
    for (e = 0; e < this.colors.length; e++) if (this.colors[e].ratio = this.colors[e].size / n, i += this.colors[e].ratio, i >= t) return this.colors[e].hex;
...
IOWA.CountdownTimer.Band.prototype.shudder = function(t) {
    if (!this.isPlaying && t) this.isPlaying = !0,
        this.fade("in"),
        this.isShuddering = !0;
...
IOWA.CountdownTimer.Core.prototype.checkTime = function() {
    ...
    if (this.isMobile && this.firstRun) {
        ...
        this.firstRun = !1
    } (this.firstRun || this.lastNumbers.days !== t.days) && (this.bands[0].changeShape(Math.floor(t.days / 10)), this.bands[1].changeShape(t.days % 10)),
    (this.firstRun || this.lastNumbers.hours !== t.hours) && (this.bands[2].changeShape(Math.floor(t.hours / 10)), this.bands[3].changeShape(t.hours % 10)),
    (this.firstRun || this.lastNumbers.minutes !== t.minutes) && (this.bands[4].changeShape(Math.floor(t.minutes / 10)), this.bands[5].changeShape(t.minutes % 10)),
    (this.firstRun || this.lastNumbers.seconds !== t.seconds) && (this.bands[6].changeShape(Math.floor(t.seconds / 10)), this.bands[7].changeShape(t.seconds % 10)),
        this.lastNumbers = t,
        this.firstRun = !1
};
IOWA.CountdownTimer.Core.prototype.onFrame = function() {
  // 关于Google IO 的图标(frame)头
    if (this.isPlaying) {
...
IOWA.CountdownTimer.Core.prototype.handleMouseShudder = function() {
    for (var t = this.mouseCoords.x,
             e = this.mouseCoords.y,
             i = 0; i < this.bands.length; i++) t > this.bands[i].center.x - this.bands[i].radius && t < this.bands[i].center.x + this.bands[i].radius && e > this.bands[i].center.y - this.bands[i].radius && e < this.bands[i].center.y + this.bands[i].radius ? this.bands[i].shudder(!0) : this.bands[i].shudder(!1);
    this.mouseCoords = null
};
...
IOWA.CountdownTimer.Core.prototype.addUnits = function() {
    ...
    e.fillStyle = "#78909C",
    e.textAlign = "center",
    e.fillText("Days", this.bands[0].center.x + this.layout.radius + this.bandPadding / 2, this.bands[0].center.y + this.layout.radius + t),
...
IOWA.CountdownTimer.Core.prototype.addSeparators = function() {
    ...
    t.save(),
        t.scale(this.pixelRatio, this.pixelRatio);
    for (var e = 0; e < this.separators.length; e++) t.clearRect(this.separators[e].x - 2, this.separators[e].y - 2, this.separators[e].w + 4, this.separators[e].h + 4),
        t.beginPath(),
...
IOWA.CountdownTimer.Core.prototype.getPath = function(t) {
    for (var e = 66,
             i = document.getElementById(t), n = i.getTotalLength(), r = this.quality, s = [], o = 0; r > o; o++) {
        var a = o * n / r,
...
IOWA.CountdownTimer.Core.prototype.resetCanvas = function() {
    ...
    this.getFormat(),
        ...
        this.getLayout(),
        this.unitsAdded = !1;
    for (var e = 0; e < this.bands.length; e++) this.bands[e].radius = this.layout.radius,
        this.bands[e].center = this.getBandCenter(e),
        this.bands[e].redraw();
    this.intro && (this.intro.radius = this.layout.radius, this.intro.center = "horizontal" === this.format ? this.getBandCenter(1) : this.getBandCenter(5)),
        this.getSeparators(),
